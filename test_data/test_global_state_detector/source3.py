...
class AuthTokenAPI(MethodView):
    ...
    def post(self):
        ...
        if user is None or not user.validate_password(password):
            return api_abort(
                code=400, message="Either the username or password was invalid."
            )
...
class CompletedItemsAPI(MethodView):
    ...
    def get(self):
        ...
        if pagination.has_prev:
            prev = url_for(
                ".completed_items", page=page - 1, _external=True
            )  # same here
        next = None
        if pagination.has_next:
            next = url_for(
...
api_v1.add_url_rule(
    "/oauth/token", view_func=AuthTokenAPI.as_view("token"), methods=["POST"]
)
api_v1.add_url_rule("/user", view_func=UserAPI.as_view("user"), methods=["GET"])
api_v1.add_url_rule(
    "/user/items", view_func=ItemsAPI.as_view("items"), methods=["GET", "POST"]
)
api_v1.add_url_rule(
    "/user/items/<int:item_id>",
    view_func=ItemAPI.as_view("item"),
    methods=["GET", "PUT", "PATCH", "DELETE"],
)
api_v1.add_url_rule(
    "/user/items/active",
    view_func=ActiveItemsAPI.as_view("active_items"),
    methods=["GET"],
)
api_v1.add_url_rule(
    "/user/items/completed",
    view_func=CompletedItemsAPI.as_view("completed_items"),
    methods=["GET", "DELETE"],
)
